...
`standard` 会话支持用户摄像头输入和屏幕共享。前端会在用户显式开启后以低帧率采样 JPEG 画面，并把最近画面附加到下一次文字提问的多模态 LLM 请求中。`voice_llm` 会话暂不接收视觉输入，也不会自动切换到 standard 管线。

浏览器摄像头和屏幕共享需要 HTTPS 或 localhost；默认采样间隔、尺寸和帧大小可通过 `pipeline.visual_input` 配置。
